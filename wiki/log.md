...
- 创建页面：
  - `来源_渐进式Spec实战指南_阿里云.md`（来源摘要）
  - `Spec Coding.md`（概念页）
- 更新页面：`Vibe Coding.md`（添加 Spec Coding 庌链接和与来源引用）、`上下文工程.md`（补充 code_copilot 静态/动态分层案例）、`Harness Engineering.md`（新增 code_copilot 个人框架与四种场景全景表）、`index.md`、`log.md`
- 关键发现：
  - **渐进式复杂度是最大亮点**：70% 的需求是小需求，强制所有需求都走完整流程本身就是偶然复杂度在吃掉效率。渐进式复杂度是对这一问题的直接回答
  - **knowledge/ 的地位比 rules/ 更重要**：大部分团队花大量精力优化 Prompt/Rules，却忽略了对 AI 输出质量上限有决定性影响的领域知识育建
  - **Reverse Sync 是最容易被忽略的核心**：文档和代码一旦脱节，整个 Spec 框架就失效。先修文档再修代码是工程约束而非个人习惯
  - **自由度曲线普遂的反向错误**：方案设计阶段应给最高自由度，执行阶段应给零自由度。大部分人反过来做——方案没想清楚就开写，执行时又让 AI 自由发挥
  - **与已有实践的互补**：code_copilot（个人/小团队）+ 白家杰 JK Launcher（个人项目）+ 腾讯 LEGO（百万行 C++）+ 腾讯审核（全链路自动化）四种规模和场景的 Harness 实践全部进入 wiki
